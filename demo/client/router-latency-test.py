...
with open(os.getenv('CAIKIT_EMBEDDINGS_CRT'), 'rb') as f:
    crt = f.read()

with open(os.getenv('CAIKIT_EMBEDDINGS_KEY'), 'rb') as f:
    key = f.read()

creds = grpc.ssl_channel_credentials(root_certificates=cacert, private_key=key, certificate_chain=crt)
